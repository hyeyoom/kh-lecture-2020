...
- [5. 제약 조건(constraint)](#5-제약-조건constraint)
  - [5.1. 제약 조건](#51-제약-조건)
  - [5.2. NOT NULL](#52-not-null)
  - [5.3. UNIQUE](#53-unique)
  - [5.4. PRIMARY KEY](#54-primary-key)
  - [5.5. FOREIGN KEY](#55-foreign-key)
...
저장할 데이터에 걸리는 특수 규칙이 제약 조건이다. 제약 조건은 DB에 저장될 데이터의 정합성 및 무결성 유지하기 위해 사용된다.  
...
CREATE TABLE 테이블_이름(
  email VARCHAR2(32) CONSTRAINT 제약조건_이름 NOT NULL
);
```
...
CREATE TABLE 테이블_이름(
  email VARCHAR2(32) CONSTRAINT 제약조건_이름 UNIQUE
);
```
...
CREATE TABLE 테이블_이름(
  email VARCHAR2(32) CONSTRAINT 제약조건_이름 PRIMARY KEY
);
```
...
```sql
CREATE TABLE 테이블_이름(
  열_이름 데이터타입 CONSTRAINT 제약_조건_이름 REFERENCES 참조할_테이블(참조할_열_이름)
);
```